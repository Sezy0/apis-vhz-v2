...
func main() {
	...
	// Initialize Log Repository (MongoDB)
	// Using the same MongoDB settings as Inventory for now, but targeting "obfuscation_logs" collection
	logRepo, err := repository.NewMongoDBLogRepository(
		cfg.InventoryDB.MongoURI,
		cfg.InventoryDB.MongoDatabase,
		"obfuscation_logs",
	)
	if err != nil {
		log.Printf("Warning: Failed to initialize MongoDB Log Repository: %v", err)
		// Fallback or handle critical failure depending on requirements. 
		// For logs, maybe non-critical is okay, or we panic.
		// Let's log warning and pass nil (handlers handle nil repo gracefully)
	} else {
		log.Println("MongoDB Log Repository initialized")
		defer logRepo.Close()
	}
